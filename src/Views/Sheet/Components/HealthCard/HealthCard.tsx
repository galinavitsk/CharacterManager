...
const HealthCard =(props: { currentHealth: number; maxHealth: number; tempHealth: number; classes: any[]; })=>{
  ...
  const onHClose = () => {
    ...
    return(<>
        ...
        <div className="HealthBar" onClick={()=>{onPToggle()}} >
            <StatusBar bColor="#9DBE9E"
                       fColor="#BAE6BC"
                       value={(props.currentHealth/props.maxHealth)*100}
                       label={GetLabelHealth(props.currentHealth,props.maxHealth,props.tempHealth)}/>
                       </div>
             {props.classes.length>=0 && 
                props.classes.map((c)=>
                <div style={{marginTop:"2px"}}>
                    <div className="HitPointsBar"  onClick={()=>{onHOpen(c.iD,c.hitDie,c.currentHDie)}}>
    ...
    aria-labelledby="contained-modal-title-vcenter"
      centered>
          <Modal.Body bsPrefix="modalContentCard">
          <HitDiceRoller classId={classId} CHD={currentHitDice} HD={hitDie} onHClose={onHClose.bind(this)}/>
        </Modal.Body>
            </Modal>
    </> );}
